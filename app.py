...
# Get Code button (redirects to GitHub repo)
github_url = "https://github.com/DhruvKarani"  # Replace with your actual repo URL

# Custom aligned buttons container
import base64
with open("requirements.txt", "rb") as req_file:
    req_bytes = req_file.read()
    req_b64 = base64.b64encode(req_bytes).decode()

github_url = "https://github.com/DhruvKarani"  # Replace with your actual repo URL
st.sidebar.markdown(
    f"""
    <div style="display: flex; flex-direction: column; gap: 0.75em; align-items: stretch; margin-bottom: 1em;">
        <a href='{github_url}' target='_blank' style='text-decoration: none;'>
            <button style='width:100%;padding:0.75em 0.5em;font-size:1.1em;background:#2563eb;color:white;border:none;border-radius:8px;box-shadow:0 2px 8px rgba(37,99,235,0.15);font-weight:600;display:flex;align-items:center;justify-content:center;'>
                <span style='font-size:1.2em;margin-right:0.5em;'>🚀</span> Get Code
            </button>
        </a>
        <a href="data:text/plain;base64,{req_b64}" download="requirements.txt" style='text-decoration: none;'>
            <button style='width:100%;padding:0.75em 0.5em;font-size:1.1em;background:#fff;color:#ef4444;border:2px solid #ef4444;border-radius:8px;box-shadow:0 2px 8px rgba(239,68,68,0.10);font-weight:600;display:flex;align-items:center;justify-content:center;'>
                <span style='font-size:1.2em;margin-right:0.5em;'>📄</span> Requirements.txt
            </button>
        </a>
    </div>
    """,
